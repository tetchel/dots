...
if ! type fswatch &>/dev/null ; then
	echo "You are missing the fswatch dependency. Install the package inotify-tools (apt-get install inotify-tools)"
	exit 1
fi
...
if [ "$#" != 1 ] ; then
	echo "Incorrect parameters. Provide exactly one directory to watch. Use --help for usage instructions."
	exit 1
fi
